...
namespace HytaleHotbar.Services
{
    public class InventoryService
    {
        ...
        public enum MoveOutcome
        {
            ...
            /// <summary>
            /// NoOp with specific indication
            /// </summary>
            FullTargetStack,

            /// <summary>
            /// Item was moved into an empty slot.
            /// </summary>
            Moved,
            ...
            /// <summary>
            /// Source stack was fully merged into the target.
            /// </summary>
            SourceStacked,

            /// <summary>
            /// Some items were merged; source still has items.
            /// </summary>
            SourcePartiallyStacked,

            /// <summary>
            /// Item was dropped outside the inventory and removed.
            /// </summary>
            Dropped,

            /// <summary>
            /// Swap situation was unable to happen but a further check could find a swap action
            /// </summary>
            UnknownCondition
        }
        ...
        public MoveOutcome TryMoveItem(int sourceIndex, int targetIndex)
        {
            ...
            // NOTE: Logic for targetIndex being null should be done in the UI (outside InventoryPanel or inside but not on a slot)
            ...
            while(keepCompacting)
            {
                ...
                else if (PlayerInventory[writePosition].Name == PlayerInventory[readPosition].Name)
                {
                    ...
                    if (readItemDef.IsStackable)
                    {
                        ...
                        {
                            // read position was fuly stacked into the write position
                    ...
                    {
                        // items are the same, but not stackable, bump by 1 positon each
                        writePosition++;
                    }
                }
                else
        ...
        private MoveOutcome CombineTwoStacks(int sourceIndex, int targetIndex)
        {
            ...
            // Load singleton defintions for each (NOTE: You may want to guard against missing key lookup errors KeyNotFoundException)
